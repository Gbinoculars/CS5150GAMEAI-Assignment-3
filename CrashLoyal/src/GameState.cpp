...
std::vector<std::shared_ptr<Mob>> GameState::buildRivers() {
	int index = 0;
	std::vector<std::shared_ptr<Mob>> riverMobs;
	for (int i = 0; i < 20; i++) {
		std::shared_ptr<Mob> r = std::shared_ptr<Mob>(new Mob_River);
		if (i >= 5 && i < 14) {
			index = i + 1;
		}
		else if (i >= 14) {
		...
		Point posR(index * 3.0f, 50.f); //0-4 5-14 15-19

		r->Init(posR, false);
		riverMobs.push_back(r);
	}
	return riverMobs;
}

std::vector<std::shared_ptr<Mob>> GameState::rivers = GameState::buildRivers();