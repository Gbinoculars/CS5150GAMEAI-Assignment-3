...
bool Mob::isMoveSamedir(std::shared_ptr<Mob> otherMob) {
	Point thisMoveDir;
	thisMoveDir.x = this->getPosition()->x + (this->getPosition()->x - this->lastPosition->x);
	thisMoveDir.y = this->getPosition()->y + (this->getPosition()->y - this->lastPosition->y);
	thisMoveDir.normalize();
	Point otherMoveDir;
	otherMoveDir.x = otherMob->getPosition()->x + (otherMob->getPosition()->x - otherMob->lastPosition->x);
	otherMoveDir.y = otherMob->getPosition()->y + (otherMob->getPosition()->y - otherMob->lastPosition->y);
...
void Mob::processCollision(std::vector<std::shared_ptr<Mob>> otherMobs, double elapsedTime) {
	for (std::shared_ptr<Building> otherBuilding : GameState::buildings) {
		if (!otherBuilding->isDead()) {
			...
			if (abs(this->getPosition()->x - otherBuilding->getPosition()->x) < averageOfSzie && abs(this->getPosition()->y - otherBuilding->getPosition()->y) < averageOfSzie) {
				std::shared_ptr<Point> pos = std::shared_ptr<Point>(new Point(this->getPosition()->x + (this->getPosition()->x - otherBuilding->getPosition()->x), this->getPosition()->y + (this->getPosition()->y - otherBuilding->getPosition()->y)));
				this->moveTowards(pos, elapsedTime);
			}
		}
	}
	for (std::shared_ptr<Mob> river : GameState::rivers) {
		...
		if (abs(this->getPosition()->x - river->getPosition()->x) < averageOfSzie && abs(this->getPosition()->y - river->getPosition()->y) < averageOfSzie) {
			std::shared_ptr<Point> pos = std::shared_ptr<Point>(new Point(this->getPosition()->x + (this->getPosition()->x - river->getPosition()->x), this->getPosition()->y + (this->getPosition()->y - river->getPosition()->y)));
	...
	if(!otherMobs.empty()){
		for (std::shared_ptr<Mob> otherMob : otherMobs) {
				if (otherMob->GetMass() > this->GetMass()) {
					std::shared_ptr<Point> pos = std::shared_ptr<Point>(new Point(this->getPosition()->x + (this->getPosition()->x - otherMob->getPosition()->x), this->getPosition()->y + (this->getPosition()->y - otherMob->getPosition()->y)));
					this->moveTowards(pos, elapsedTime);
				}
				else if (otherMob->GetMass() == this->GetMass()) {
					std::shared_ptr<Point> pos = std::shared_ptr<Point>(new Point(otherMob->getPosition()->x + (otherMob->getPosition()->x - this->getPosition()->x), otherMob->getPosition()->y + (otherMob->getPosition()->y - this->getPosition()->y)));
					std::shared_ptr<Point> pos1 = std::shared_ptr<Point>(new Point(this->getPosition()->x + (this->getPosition()->x - otherMob->getPosition()->x), this->getPosition()->y + (this->getPosition()->y - otherMob->getPosition()->y)));
					if (this->isMoveSamedir(otherMob)) {
						if (this->isBehind(otherMob)) {
							this->moveTowards(pos1, elapsedTime);
						}
						else {
							otherMob->moveTowards(pos, elapsedTime);
						}
					}
					else {
						this->moveTowards(pos1, elapsedTime);
