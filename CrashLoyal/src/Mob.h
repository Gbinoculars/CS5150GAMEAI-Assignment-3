...
protected: 
	...
	std::shared_ptr<Point> lastPosition;

	static std::string previousUUID;

	std::string uuid;

	//std::vector<std::shared_ptr<Mob>> buildingsMobs;

	//std::vector<std::shared_ptr<Mob>> riverMobs;
	...
	bool checkIsInAttackRange(std::shared_ptr<Attackable> otherMob);
	...
	void moveTowardsVector(std::shared_ptr<Point> vector, double elapsedTime);
	// Movement related
	//////////////////////////////////
	// Combat related
	...
	void processCollision(std::vector<std::shared_ptr<Mob>> otherMobs, double elapsedTime);
	...
	bool isBehind(std::shared_ptr<Mob> otherMob);

	bool isMoveSamedir(std::shared_ptr<Mob> otherMob);
